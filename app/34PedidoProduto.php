  ...
  <div class="container-fluid"><br>
    ...
    <div class="row g-1" id="entradaProduto">
      ...
      <div class="col-md-2">
        ...
        <input style="font-size: 16px; color:yellow; text-align:right; background: rgba(0,0,0,0.3)" type="text" class="form-control" id="" name="" value="<?php echo number_format($_SESSION['qtdeLote'], 0,  ',', '.') . ' ' . $rowProduto['UNIDADE'] ?>" readonly>
      </div>
      <div class="col-md-8">
      ...
      <div class="overflow-auto">
        <table class="table table-dark table-hover">
          <thead style="font-size: 12px">
            <tr>
              <th scope="col" style="width: 30%; color: gray"                     >Descrição do Material</th><th scope="col" style="width: 10%; color: gray; text-align: center;">Qtde Necessária    </th>
            ...
            while($rowLista = $query_material->fetch(PDO::FETCH_ASSOC)){ $contador = 1; $capacidadeProcess = $rowLista['CAPAC_PROCESS'];?>
              <tr>
                ...
                <td scope="col" style="width: 10%; text-align: center; font-size: 18px; color:yellow"><?php
                  ...
                  $query_Estoque->bindParam(':descrMat', $descrMaterial, PDO::PARAM_STR); $query_Estoque->execute(); $resultEstoque = $query_Estoque->fetch(PDO::FETCH_ASSOC);
    ...
    <form method="POST" id="dataSelecionada">
      <div class="row g-2">
        ...
        <div class="col-md-7"><br><!-- Button trigger modal -->
          <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#calendario">Verificar Disponibilidade na fila da planta de fabricação</button>
        </div>
        <!-- Modal -->
    ...
    <form method="POST">
      <div class="row g-2"><?php
        if(!empty($dataLivre['agendar']) || !empty($dataLivre['dataSelecionada'])){ $dataConvert = date('Y-m-d', strtotime($dataLivre['dataSelecionada'])); $convert1 = time(); $convert2 = strtotime($dataConvert);  
          ...
          </div><div class="col-md-1"></div><br><div class="col-md-1"></div>
          <div class="col-md-2">
