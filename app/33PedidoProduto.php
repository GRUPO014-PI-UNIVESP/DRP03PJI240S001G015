    ...
    <form method="POST">
      <div class="row g-3">
        <div class="col-md-3">
          ...
          <div class="input-group mb-2">
            <input type="number" class="form-control" id="padrao" name="padrao" 
                   style="font-size: 13px; text-align: center; background: rgba(0,0,0,0.3)" value="3">
            <span class="input-group-text" style="font-size: 13px">dias em média</span>
          </div><p style="font-size:11px; color:grey">Tempo para fabricação e análises de qualidade</p>
        </div>
        <div class="col-md-3">
          ...
          <div class="input-group mb-2">
            <input type="number" class="form-control" id="xtend" name="xtend" 
                   style="font-size: 13px; text-align: center; background: rgba(0,0,0,0.3)" value="6">
            <span class="input-group-text" style="font-size: 13px">dias em média</span>
        ...
        <div class="col-md-2"> <?php 
          ...
          $queryLast->execute(); 
          $rowID = $queryLast->fetch(PDO::FETCH_ASSOC); 
          $novoID = $rowID['ULTIMO'] + 1;?>
      ...
      if(!empty($produto['pedido']) && $produto['nomeProduto'] != 'Selecione o Produto'){
        ...
        while($rowLista = $query_material->fetch(PDO::FETCH_ASSOC)){
          ...
          $query_estoque = $connDB->prepare("SELECT ID_ESTOQUE, SUM(QTDE_ESTOQUE) AS TOTAL_ESTOQUE FROM materiais_estoque 
                                                    WHERE DESCRICAO = :material");
          ...
          $query_reserva = $connDB->prepare("SELECT NUMERO_PEDIDO, SUM(QTDE_RESERVA) AS TOTAL_RESERVA FROM materiais_reserva 
          ...
          if($alerta == 'DISPONÍVEL'){
            $disp = 3;
            $veriEstoque = $connDB->prepare("SELECT ID_COMPRA FROM materiais_lotes 
                                                    WHERE ETAPA_PROCESS = 3 AND QTDE_LOTE > :qtdePedido ORDER BY ID_INTERNO ASC");
            $veriEstoque->bindParam(':qtdePedido', $_SESSION['qtdeLote'], PDO::PARAM_STR);
            $veriEstoque->execute();
            $rowLote = $veriEstoque->fetch(PDO::FETCH_ASSOC); 
          ...
          if($alerta == 'INSUFICIENTE'){ 
            ...
            $buscaIDcompra = $connDB->prepare("SELECT ID_COMPRA FROM materiais_compra 
                                                      WHERE NUMERO_PEDIDO = :numPedido AND ID_ESTOQUE = :idEstoque");
            ...
            $buscaIDcompra->execute();
            $idCompra = $buscaIDcompra->fetch(PDO::FETCH_ASSOC);
