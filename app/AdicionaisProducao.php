...
<style>
  .tabela{ width: 100%; height: 480px; overflow-y: scroll;} 
</style>
<!-- Área Principal -->
<div class="main">
    ...
    <div class="row g-1">
      <div class="col-md-2">
        ...
        <input style="font-weight: bold; font-size: 18px; background: rgba(0,0,0,0.3); text-align: center; width: 160px;" 
               type="number" class="form-control" id="nPedido" name="nPedido" autofocus onclick="submeterFormulario()">
      </div>
    </div>
  </form><?php
  $captaTab = filter_input_array(INPUT_POST, FILTER_DEFAULT);
  if(isset($_POST['nPedido'])){ $proced = intval($captaTab['novaTabela']);
    $selTab = $connDB->prepare("SELECT * FROM estrutura WHERE PROCEDIMENTO = 4 AND ATIVO = 1");
    $selTab->execute(); $rowTab = $selTab->fetch(PDO::FETCH_ASSOC);
    $callStruc = $connDB->prepare("SELECT * FROM estrutura_campos WHERE PROCEDIMENTO = 4 AND ATIVO = 1");
  }
  ?>
</div>