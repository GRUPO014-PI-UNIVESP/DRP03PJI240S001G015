...
<div class="main">
  <div class="container-fluid">
    ...
    <div class="tab-content" id="myTabContent">
      ...
      if(!empty($_GET['id'])){
        $dadosPedido = $connDB->prepare("SELECT * FROM pedidos WHERE ID_PEDIDO = :idPedido"); $dadosPedido->bindParam(':idPedido', $_GET['id'], PDO::PARAM_INT);
        $dadosPedido->execute(); $rowPedido = $dadosPedido->fetch(PDO::FETCH_ASSOC); $dataRegistro = date('Y-m-d'); $dV = date('Y-m-d', strtotime($rowPedido['DATA_FABRI']));
        $validade = date('Y-m-d', strtotime($dV."+ 6 months"));$_SESSION['nClnt'] = $rowPedido['CLIENTE'];
        $_SESSION['nProd'] = $rowPedido['PRODUTO']; $_SESSION['nLotePF'] = $rowPedido['NUMERO_LOTE']; $_SESSION['qLote'] = $rowPedido['QTDE_PEDIDO'];
        $_SESSION['dataR'] = date('Y-m-d', strtotime($dataRegistro)); $_SESSION['dataV'] = date('Y-m-d', strtotime($validade));
      ...
      <div class="tab-pane fade show active" id="entrada-tab-pane" role="tabpanel" aria-labelledby="entrada-tab" tabindex="0"><br>
        <div class="row g-1">
          ...
          <div class="col-md-2">
            <div class="form-floating mb-3">
              <input type="text" class="form-control" id="nLoteInterno" name="nLoteInterno" style="font-weight: bolder; text-align: center; background: rgba(0,0,0,0.3); color: yellow" value="<?php echo $rowPedido['NUMERO_LOTE'] ?>" readonly>
          ...
          <div class="col-md-2">
            <div class="form-floating mb-3">
              <input type="text" class="form-control" id="dataFabri" name="dataFabri" style="font-weight: bolder; text-align: center; background: rgba(0,0,0,0.3); color: yellow" value="<?php echo date('d/m/Y', strtotime($rowPedido['DATA_FABRI'])) ?>" readonly>
          ...
          <div class="col-md-2">
            <div class="form-floating mb-3">
              <input type="text" class="form-control" id="dataVali" name="dataVali" style="font-weight: bolder; text-align: center; background: rgba(0,0,0,0.3); color: yellow" value="<?php echo date('d/m/Y', strtotime($validade)) ?>" readonly>
          ...
          if(!empty($confirma['confirma'])){ $_SESSION['contami'] = $confirma['contaminantes']    ; $_SESSION['confirma'] = $confirma['confirma']; $_SESSION['pureza'] = $confirma['pureza'];           
            $_SESSION['cor']  = $confirma['cor'] ; $_SESSION['aspecto'] = $confirma['aspecto']    ; $_SESSION['observ'] = $confirma['observacao']; $_SESSION['scalaPH'] = $confirma['escalaPH'];
            $_SESSION['odor'] = $confirma['odor']; $_SESSION['perdaM']  = $confirma['perdaMassa'] ;
