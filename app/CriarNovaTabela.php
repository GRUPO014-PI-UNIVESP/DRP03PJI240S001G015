...
<div class="main"><br>
  ...
  <form action="" id="criar" method="POST">
    <div class="row g-2">
      <div class="col-md-4">
        ...
        <p style="font-size: 10px; color: grey">Tamanho máximo de 25 caracteres (sem espaço e sem acento)</p>
      </div>
      <div class="col-md-8">
        <?php
  ...
  if(isset($_POST['criar'])){
    if(!empty($criarTabela)){
      ...
      $nomeTabela = 'e_' . strtolower($criarTabela['novoNome']);
      $novoID = 'ID_' . strtoupper($nomeTabela);
      $descr = $criarTabela['descr'];
      ...
      if(!empty($rowEstrutura['NOME_TABELA'])){
        ...
        if($rowEstrutura['NOME_TABELA'] != $nomeTabela){
          ...
          try{
            ...
            for($i = 1; $i <=3; $i++){
              $regCampo = $connDB->prepare("INSERT INTO estrutura_campos (ID_ESTRUTURA, ETIQUETA, CODIGO, TIPO, CAMPO, DESCRICAO) 
                                                   VALUES (:idStruc, :etiq, :code, :tipo, :campo, :descr)");
              $regCampo->bindParam(':idStruc', $_SESSION['ID_TABELA'], PDO::PARAM_INT);
              $regCampo->bindParam(':etiq'   , $e[$i]                , PDO::PARAM_STR);
              $regCampo->bindParam(':code'   , $b[$i]                , PDO::PARAM_STR);
      ...
      if(empty($rowEstrutura['NOME_TABELA'] )){
        ...
        try{
          ...
          echo 'Ocorreu algum problema durante a criação da tabela, comunique o responsável do TI.'?>
          <div>
            <br><br>
            <button class="btn btn-danger" onclick="location.href='./ConfigurarEstrutura.php'">Reiniciar</button>
          </div><?php
