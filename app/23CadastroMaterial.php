...
include_once './ConnectDB.php'; include_once './EstruturaPrincipal.php'; $_SESSION['posicao'] = 'Cadastro de Material'; include_once './RastreadorAtividades.php';
...
<div class="main">
  <div class="container-fluid"><br> 
    <div class="row g-4"><div class="col-md-6"><h5>Cadastro de Novo Material</h5></div><div class="col-md-4"></div>
      <div class="col-md-2">
        <button class="btn btn-warning" style="width: 100px" onclick="location.href='./00SeletorAdministrativo.php'">Sair</button>
      </div>
    </div>
    <form method="POST" id="cadastroMaterial">
      <div class="col-md-10">
      ...
      <div class="row g-2">
        <div class="col-md-3">
          ...
          <select style="font-size: 12px;" class="form-select" id="uniMed" name="uniMed">
            <option selected>Selecione</option><option value="KG">KG</option><option value="LT">LT</option><option value="UN">UNIDADE</option>
          </select>
        </div>
        ...
        <div class="col-md-3" style="padding: 3px;"><br>
          <input style="width: 140px;" class="btn btn-secondary" type="reset" id="reset" name="reset" value="Descartar" onclick="location.href='./23CadastroMaterial.php'">
        </div>
      </div>
    </form><?php
    $salvar = filter_input_array(INPUT_POST, FILTER_DEFAULT);
    if(!empty($salvar['salvar'])){ $qtdeLote = 0;
      $descrMat   = strtoupper($salvar['descrMaterial']);
      $fornecedor = strtoupper($salvar['fornecedor']);
      $uniMed     = strtoupper($salvar['uniMed']);

      $buscaID = $connDB->prepare("SELECT MAX(ID_ESTOQUE) AS ULTIMO FROM materiais_estoque");
      $buscaID->execute(); $rowID = $buscaID->fetch(PDO::FETCH_ASSOC); $novoID = $rowID['ULTIMO'] + 1;

      $regMaterial = $connDB->prepare("INSERT INTO materiais_estoque (ID_ESTOQUE, FORNECEDOR, DESCRICAO, UNIDADE) VALUES (:idEstoque, :fornecedor, :descrMat, :uniMed)");
      $regMaterial->bindParam(':idEstoque' , $novoID    , PDO::PARAM_INT);
      $regMaterial->bindParam(':fornecedor', $fornecedor, PDO::PARAM_STR);
      $regMaterial->bindParam(':descrMat'  , $descrMat  , PDO::PARAM_STR);
      $regMaterial->bindParam(':uniMed'    , $uniMed    , PDO::PARAM_STR);
      $regMaterial->execute();
      header('Location: ./23CadastroMaterial.php');
    } ?>
  </div>
</div>