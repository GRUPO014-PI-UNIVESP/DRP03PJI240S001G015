...
<div class="main">
  <div class="container-fluid"><br>
    <form method="POST">
      <div class="row g-3">
        ...
        <div class="col-md-7">
          ...
          <select style="font-size: 12px;" class="form-select" id="descrMat" name="descrMat">
            <option style="font-size: 12px" selected>Selecione o Material</option><?php
              ...
              while($rowMat = $query_material->fetch(PDO::FETCH_ASSOC)){?>
                <option style="font-size: 12px"><?php echo $rowMat['DESCRICAO']; ?></option> <?php
              }?>
          </select>
        </div>
    ...
    if(!empty($confirmaCompra['agendado'])){ 
      ...
      $dataPedido = date('Y-m-d', strtotime($confirmaCompra['dataPedido'])); $dataPrazo = date('Y-m-d', strtotime($dataPedido."+ 7 days"));
      ...
      $realiza = $connDB->prepare("INSERT INTO materiais_lotes (ID_ESTOQUE, DESCRICAO, QTDE_LOTE, UNIDADE, ETAPA_PROCESS, SITUACAO) VALUES (:idEstoque, :descrMat, :qtdeLote, :uniMed, :etapa, :situacao)");                                               
      ...
      $realiza->bindParam(':situacao', $situacao  , PDO::PARAM_STR); $realiza->bindParam(':uniMed'   , $rowBusca['UNIDADE']   , PDO::PARAM_STR);$realiza->execute();
